package com.longtailvideo.jwplayer.media {
	...
	public class VideoMediaProvider extends MediaProvider {
        ...
        private var _lastBytes:Number = 0;
		...
		protected function positionHandler():void {
			...
			if (_buffered < 100) {
                var bytesTotal = _playerConfig.estimatedBytes > 0 ? _playerConfig.estimatedBytes : _stream.bytesTotal;
				var _b1:Number = Math.floor(100 * (_stream.bytesLoaded/bytesTotal + _offset.time/_item.duration));
                var _b2:Number = Math.floor(100 * (_stream.bytesLoaded/_stream.bytesTotal + _offset.time/_item.duration));
				_buffered = Math.min(100, Math.max(_b1, _b2));
				sendBufferEvent(_buffered);
                _lastBytes = _stream.bytesTotal;
			}
		...
		override public function seek(pos:Number):void {
            ...
            var range_start:Number = _playerConfig.timeOffset;
            var range_end:Number = _playerConfig.timeOffset + ((_item.duration - _playerConfig.timeOffset) * _buffered / 100);
            // In case buffering is somehow behind the actual position
            range_end = Math.max(range_end, _position);
            clearInterval(_interval);
            
            if(pos >= range_start && pos <= range_end) {
                // We have to adjust for this because if we have an offset of 300
                // And the user seeks to 303, that's really only 3 seconds according
                // to the internal data for jwplayer
                pos = pos - _playerConfig.timeOffset;
                _position = pos;
                rval = _stream.seek(_position);
            } else {
                rval = false;
            }

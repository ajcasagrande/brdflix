			...
			switch (evt.type) {
				...
				case MediaEvent.JWPLAYER_MEDIA_TIME:
					if (evt.duration == -1 && evt.type == MediaEvent.JWPLAYER_MEDIA_TIME) {
						...
						if (scrubber) {
							var pos:Number = _player.config.timeOffset + evt.position;
                            var offset:Number = _player.config.timeOffset / _player.config.runtime * 100;
                            if (evt.duration <= -60) {
								scrubber.setProgress((evt.duration - pos) / evt.duration * 100);
							}
							else {
								scrubber.setProgress(pos / evt.duration * 100);
                                if (_player.config.timeOffset > 0){
                                    (scrubber as TimeSlider).removeCues();
                                    (scrubber as TimeSlider).addCue(_player.config.timeOffset, "");
                                }
							}
							...
							if (evt.bufferPercent > 0) {
                                ...
                                var offsetPercent:Number = (_player.config.timeOffset / _player.config.runtime) * 100;
                                scrubber.setBuffer(offsetPercent + (evt.bufferPercent * (100 - offsetPercent) / 100), offsetPercent);
							}
						}
						...
						if (pos > 0 || evt.duration <= -60) { setTime(pos, evt.duration); }

package com.longtailvideo.jwplayer.view.components {
	...
	public class Slider extends Sprite {
        ...
        protected var _progressOffset:Number = 0;
		...
		public function setProgress(progress:Number, offset:Number=NaN):void {
			if (_dragging) return;
			if (isNaN(progress)) {
				progress = 0;
			}
            ...
            if (!isNaN(offset)) {
                _progressOffset = Math.min(Math.max(offset, 0), 100) / 100;
            } else {
                _progressOffset = 0;
            }
		...
		public function resize(width:Number, height:Number):void {
			...
			if (!_dragging) {
				resizeSlider(_currentProgress, _progressOffset, offset, _progress, _progressCapLeft, _progressCapRight);
			}

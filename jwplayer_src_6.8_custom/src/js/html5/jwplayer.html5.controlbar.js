...
(function(jwplayer) {
	...
	html5.controlbar = function(api, config) {
        ...
        function _setDuration(dur){
            console.log("Setting duration: " + dur);
            _duration = dur;
        }
        function _setTimeOffset(offset){
            console.log("Setting timeOffset: " + offset);
            _timeOffset = offset;
        }
		...
		function _timeUpdated(evt) {
			...
			//} else {
                evt.duration = jwplayer().getDuration();

                var pos = evt.position + jwplayer().getTimeOffset();

                var timeString;
				if (_elements.elapsed) {
					timeString = utils.timeFormat(pos);
